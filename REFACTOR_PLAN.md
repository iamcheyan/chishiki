...
### 8.18 第十五轮（cee080c→）：spec 残余三路径终扫（冲突/锚点跳转/⌘N）

**动机**：对照任务书功能清单逐项盘点，最后三条 spec 路径未在最终代码实证：外部 mtime 冲突检测（#4 末项）、搜索结果章节锚点跳转（#5 后半）、⌘N 新建+Esc 层层退出（#7）。

**冲突检测两路径（轮询 20s）**：
- 再読込——编辑会话中外部改写 → ≤20s 轮询检出横幅「他の場所でこのドキュメントが更新されました」→ 再読込 → ta 更新为外部内容、横幅消失 ✅（首段测试因 30s 工具上限截断，但 ta 值成为外部内容即完整证据；`finally` 删除被 404 轮询吞掉属设计内「文档可能被删」）
- 保持——本地脏 `my local edit` + 外部改写 → 横幅 → このまま保持 → 横幅消失且**本地编辑保留**（baseMtime 更新为服务端，下次保存覆盖）✅

**搜索锚点跳转**：「手順」3 结果行全带章节标签 → 点击 → hash `#/doc/…?h=復旧手順` → `#復…`# 復旧手順` 元素存在、滚动至视口顶部、面板关闭、阅读视图 ✅（注：结果行是 div+gotoDoc click handler 而非 `<a href>`，选择器须按 `.sr-line`）。

**⌘N / Esc**：⌘N 弹目录菜单（6 项）→ Esc 关闭 ✅（层层退出链）。
...
### 8.22 第十九轮（e561bcf→）：仓库拓扑确认 + git 面板数据流闭环

**拓扑确认**：`docs/` 非独立 git 仓库（`git -C docs rev-parse --show-toplevel` = 主仓根）——文档数据与代码同仓同 remote，数据提交无分离焦虑；v1 的 build_search_index.py 工具虽残留但与现行 API 无关（§8.19 已记）。

**git 面板最终态数据流**：当前文档打开 → btn-git → 面板显示「コミット済み・変更なし ✓」+ 该文档历史（8c228d6）+ この版に戻す按钮；dirty=0 时「すべてコミット」按钮正确隐藏 ✅。与 API 状态完全一致。提交链路（dirty>0 → ボタン → 消息对话框 → /api/git/commit → rc 校验）已在 §8.7/§8.8 实证（含 add -A、rc≠0→500）。
